...
(function () {
  ...
  let ctx, canvas;
  let level, onEnd;
  let player, tokens, enemies, walls, door, portals, rocks, books, hazards, checkpoints, questionGates, chests, boss;
  let routeCells;
  ...
  window.DiamondRush = {
    start(lvl, cb, gameConfig) {
      ...
      player = {
        gx: 1, gy: 1,
        px: CELL, py: CELL,
        dir: 'right', moving: false, moveT: 0,
  ...
  function generateMap() {
    ...
    hazards = []; checkpoints = []; questionGates = []; chests = []; boss = null;
    routeCells = new Set();
    ...
    for (let i = 0; i < hazardCount; i++) {
      const cell = randomEmptyCell();
      if (cell) hazards.push({ gx: cell.x, gy: cell.y, type: i % 2 === 0 ? 'fire' : 'spikes', active: true, phase: Math.random() * Math.PI * 2 });
    }
  ...
  function tryMove(dir) {
    ...
    if (nx === door.gx && ny === door.gy) {
      ...
      addMessage(boss && !bossDefeated ? '¡Derrota al guardián y reúne los conceptos!' : '¡Recoge todos los conceptos correctos!', door.gx, door.gy, '#ff5c5c');
      return;
    }
  ...
  function drawEnemy(e) {
    ...
    egrad.addColorStop(0, e.type === 'snake' ? '#9edb5a' : '#e884ff'); egrad.addColorStop(1, e.type === 'snake' ? '#3f6b2c' : '#6e3c8c');
    ctx.fillStyle = egrad;
    ctx.beginPath();
  ...
  function drawPlayer() {
    ...
    pgrad.addColorStop(0, '#ffe888'); pgrad.addColorStop(1, '#c89b32');
    ctx.fillStyle = pgrad;
    ctx.beginPath();
